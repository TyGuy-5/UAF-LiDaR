...
\subsection{Lab Methods}

Razor blade: In order to fully profile the beam waist I used a razorblade, on an oplitcal stage to incrementally block a growing portion of the beam as to then reconstruct the power distribution of the beam. This method
relys on the razor blade being moved exactly perpendicular to its own orientation. I achived this by taking a mounting plate 
and laying it flat, 90 degrees from the orientation it will be mounted. I then set it atop a machinests square as to know 
the edge of the bracket is perfectly parallel tot he table. Lining it flush with the edge of the black I could then place a 
razorblade, edge flush with the table and back end resting on the mounting bracket.I then seccured this position with kapton 
tape and continued to assemble the rest of the stage.    

Laser Warming: At the begining of each test, I set up the razorblade configuration, and then turn on the laser. I let the laser warm up and set for about 15 minutes before I turn on the amplifier. I let that warm up and settle for about an hour before I find the edge of the beam, but advancing the razor blade until the power drops, and then backing it up a bit as to capture the firinge. 

For the first trial I set up the experement, following x figure, with Michael Rodewigg. Ether due to measurement error or 
components drifting over the weekend, when I came back to take data I noticed the distanced between compentets added up 
to a differece of  438 - 381.89  = 56.11 mm, from the caluclated distances require to find the waist. I did not however notice this
discrepancy unitll after data had been collected so I anyalised it anyway.

For the second trial I remeasured and adjusted the componets as best I could to fit the theoretical calculated measurements. 


For the third test, I used an non particularly straight forward aproach. Due to teh  limitations of the oiwer meter , I one wishes to manulally record specific measurements of both sensors, to trackand attemot to midigate power fluctuations in the laser, one can only sift back through recorded data as there soes not seem to eb a screen mide ti view in time measuremnts with an amount of meaningful precision.
To get around this I started the power meter recording and a stopwatch, precise up to 10 ms at the same time. Then I would record lap times for each micron, letting the meter settle for roughly 40s ever time it would have issuse with my shirt, spiking the readings. 
I then went mannually through the 660,000 data points, and extrsacted all useable data, matching the lap times to the 10micro second intervals of the recording.

For the fourth test (verticle version of test 3) I followed procedure from test 3, with 960,000 data points and 192 points of interest.
For the fifth test I followed procedure from test 3, with 1.4 million
 data points and 721 points of interest.
...
\begin{table}[H]
    ...
    \begin{tabular}{lcc}
        ...
        \midrule
            ...
            75\%  & 1.091096
    \end{tabular}
    \caption{Statistical Summary of Ambient Beam Power}
\end{table}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{"figures/stat.png"}
\end{figure}
...
\begin{itemize}
    ...
    \item Initiate appropriate music... good fall backs are Led Zeppelin, Black Sabbath, or AC/DC. 
\end{itemize}
