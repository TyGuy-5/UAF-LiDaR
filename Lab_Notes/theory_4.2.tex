...
\usepackage{kpfonts}
\usepackage[kerning=true, tracking=true, spacing=true, factor=1500, stretch=50, shrink=50]{microtype}
...
\textbf{Radius of Curvature ($R_g$):} The radius of curvature is defined by Eq.~\eqref{roc}. At its boundaries, $\lim_{z\to 0} R_g(z)=\infty$ where the wavefront is perfectly flat and acts like a section of a circle with infinite radius, and $\lim_{z\to\infty}R_g(z)=z$ where the wavefront averages out to a radius equal to the distance from the waist.
...
\emph{Given $L/b=2.84$ and $L=50$~mm.}
...
\emph{Given $L/b=2.81$ and $L=50$~mm.}
...
\emph{Repeating the analysis without assuming the beam waist is at the collimator face.}\footnote{Note: 1.21~mRad was a measured value, substantially lower than the spec sheet value of approximately 5~mRad.}

\begin{enumerate}
    \item Using Eq.~\eqref{theta} to recover the waist from the measured divergence:
$$\omega_{01}=\frac{\lambda}{\pi\theta}=\frac{1.064\times10^{-6}}{\pi\cdot 0.00121}\approx 279.9~\mu\text{m}.$$

    \item Using the beam propagation equation (Eq.~\eqref{w(z)}, from Eq.~2-27 in \cite{Osche}):
...
Setting $\omega(z)=0.00028$~m (the $\tfrac{1}{e^2}$ radius at the collimator aperture):
...
The $q$-parameter transforms as (Eq.~\eqref{q2} from \cite{Osche}):
...
$$d_2 = f + \alpha^2(d_1 - f).$$
proving Eq.~\eqref{22}, where $\alpha^2 = \frac{f^2}{(d_1-f)^2+z_{01}^2}$ follows Eq.~\eqref{18}.
\end{enumerate}
