...
\usepackage{longtable}  % Tables that span multiple pages (temperature log)
\usepackage{endnotes}
\let\footnote=\endnote

% Blank field for lab note-taking (default width 3 cm, override with optional arg)
...
\begin{abstract}
This document presents the theoretical framework and optical prescription for optimizing
second-harmonic generation (SHG) at 1064~nm using a periodically poled lithium niobate
(PPLN) crystal ($50 \times 2 \times 0.5$~mm$^3$).
Key Gaussian beam parameters---beam waist, Rayleigh range, divergence, and confocal
parameter---are defined and applied to determine the optimal focusing geometry.
The Boyd--Kleinman condition for maximum CW SHG efficiency ($L/b = 2.84$) yields a
target focused waist of approximately 54.6~$\mu$m; practical system constraints favor a
working target of 77--80~$\mu$m ($L/b \approx 1.3$--$1.4$).
Starting from a collimated 0.56~mm beam at 1064~nm with a measured divergence of
1.21~mRad, the true collimator waist is estimated to be $\omega_{01} \approx 280$~$\mu$m,
located 6.2~mm behind the collimator face.
ABCD ray-transfer matrix formalism yields closed-form expressions relating a chosen lens
focal length $f$ to the required lens position $d_1$ and image distance $d_2$, with
magnification $\alpha \approx 0.286$.
A lab notes section is included for recording setup parameters, power budgets, temperature
sweeps, and run-by-run data during crystal characterization.
\end{abstract}
...
d_2 = f + (0.28582)^2(d_1-f)$$
    ...
    \item \textbf{Alternative derivation via complex beam parameter.} Starting from \cite{Osche}:
...
Setting the real part to zero (waist condition: $\lim_{z\to 0}1/R_g(z)=0$) and simplifying:
$$d_2 = \frac{f\!\left[(d_1-f)^2 + z_{01}^2\right] + f^2(d_1-f)}{(d_1-f)^2+z_{01}^2}
= f + \frac{f^2(d_1-f)}{(d_1-f)^2+z_{01}^2}$$
$$\boxed{d_2 = f + \alpha^2(d_1 - f)}$$
recovering Eq.~\ref{22}, with $\alpha^2 = f^2/[(d_1-f)^2+z_{01}^2]$ consistent with Eq.~\ref{18}.
\end{enumerate}
...
\caption{S1: Laser at 1064\,nm.
         S2: [Component --- fill in].
         S3: [Component --- fill in].
         S4: [Component --- fill in].
         S5: [Component --- fill in].}
\label{fig:system}
\end{figure}
...
\begin{tabular}{@{}ll@{}}
\textbf{Date / Time:}             & \blank[6cm] \\
\textbf{Operator:}                & \blank[6cm] \\
\textbf{Laser power at source:}   & \blank[4cm]~mW \\
\textbf{Crystal lot / grating period:} & \blank[5cm] \\
\textbf{Session notes:}           & \blank[7cm] \\
\end{tabular}
...
\begin{itemize}
  \item[$\square$] Polarization verified (e-polarized, axis parallel to crystal \emph{thickness})
  \item[$\square$] Temperature controller connected; setpoint stable at target $\pm 0.1^{\circ}$C
  \item[$\square$] Lens installed; $d_1$ and $d_2$ set per calculated values below
  \item[$\square$] Crystal positioned at beam focus
  \item[$\square$] All power meters calibrated / zeroed
  \item[$\square$] Beam profiler available (if verifying $\omega_{02}$ directly)
  \item[$\square$] Laser safety interlocks and enclosure verified
  \item[$\square$] Dichroic / filter for 532~nm in place at output
\end{itemize}
...
\toprule
\textbf{Parameter} & \textbf{Target} & \textbf{Calculated} & \textbf{Measured} \\
\midrule
Lens focal length $f$                  & TBD              & \blank[2cm] & \blank[2cm] \\
Magnification $\alpha$                 & $0.2858$         & $0.2858$    & \blank[2cm] \\
$d_1$ (collimator waist to lens)       & ---              & \blank[2cm] & \blank[2cm] \\
$d_2$ (lens to crystal entrance)       & ---              & \blank[2cm] & \blank[2cm] \\
Focused waist $\omega_{02}$            & $77$--$80~\mu$m  & \blank[2cm] & \blank[2cm] \\
$L/b$ ratio                            & $1.32$--$1.43$   & \blank[2cm] & \blank[2cm] \\
\bottomrule
\end{tabular}
\renewcommand{\arraystretch}{1}
...
\toprule
\textbf{Measurement Location} & \textbf{Power (mW)} & \textbf{Notes} \\
\midrule
Laser source (before collimator)        & \blank & \\
After collimator                        & \blank & \\
After isolator / wave plates            & \blank & \\
Immediately before PPLN crystal         & \blank & \\
Fundamental (1064~nm) after crystal     & \blank & \\
SHG output (532~nm) after crystal       & \blank & \\
Conversion efficiency $\eta = P_{532}/P_{1064,\text{in}}$ & \blank & \\
\bottomrule
\end{tabular}
\renewcommand{\arraystretch}{1}

% ---
\subsection*{Temperature Sweep Log}
\noindent
Previous best: 57~mW at $59.5^{\circ}$C.
Recommended sweep: heat to $\sim\!20^{\circ}$C \emph{above} the expected phase-matching
temperature, then cool slowly while logging power at each step.\\[6pt]

\renewcommand{\arraystretch}{1.5}
\begin{longtable}{@{}p{4.5cm}p{4.5cm}p{5cm}@{}}
\toprule
\textbf{Crystal Temp ($^{\circ}$C)} & \textbf{SHG Power (mW)} & \textbf{Notes} \\
\midrule
\endfirsthead
\toprule
\textbf{Crystal Temp ($^{\circ}$C)} & \textbf{SHG Power (mW)} & \textbf{Notes} \\
...
% ---
\subsection*{Run Log}
\noindent One row per significant optical change (lens swap, rail position adjustment, power level, etc.).\\[6pt]

\renewcommand{\arraystretch}{1.8}
\begin{tabular}{@{}cccccc p{4.5cm}@{}}
\toprule
\textbf{Run} & \textbf{$f$ (mm)} & \textbf{$d_1$ (mm)} & \textbf{$d_2$ (mm)} & \textbf{$T$ ($^{\circ}$C)} & \textbf{Peak SHG (mW)} & \textbf{Notes} \\
\midrule
1 & & & & & & \\[10pt]
2 & & & & & & \\[10pt]
3 & & & & & & \\[10pt]
...
\vspace{0.4cm}
\noindent\rule{\linewidth}{0.4pt}\\[20pt]
\noindent\rule{\linewidth}{0.4pt}\\[20pt]
\noindent\rule{\linewidth}{0.4pt}\\[20pt]
\noindent\rule{\linewidth}{0.4pt}\\[20pt]
\noindent\rule{\linewidth}{0.4pt}\\[20pt]
